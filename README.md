...
4. 如有需要，从 [HANDOFF.example.md](HANDOFF.example.md) 复制本地 `HANDOFF.md`，填写目标考试与每日投入时间。

### 零依赖路径

临时环境或首次试用时，只用 Markdown 也能走通最小闭环：

1. 从 [HANDOFF.example.md](HANDOFF.example.md) 复制本地 `HANDOFF.md`，填写目标、考试日期和每日投入时间。
...
| 当前交接与目标配置 | 本地 `HANDOFF.md`（模板见 [HANDOFF.example.md](HANDOFF.example.md)） |
| Skill 定义 | `skills/` |
